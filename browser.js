...
function watchDirectory (bool) {
	if (bool) {
		...
		gaze = null;
		gaze = new Gaze('*.jade', {cwd: options.watch.directory}, function (err, watcher) {
			this.on('changed', function (filepath) {
				console.log(filepath + ' was changed');
				var dest = getDestination(filepath);
				start = +new Date();
				readJadeFile(filepath, dest, compileJade);
			});
...
holder.ondrop = function (e) {
	...
	if ( files.length === 1 ) {
		...
		if ( extension === '.jade' ) {
			readJadeFile(file.path, dest, compileJade);
		} else {
			changeInfoText("File was not a Jade file.");
		};
	} else if ( files.length > 1 ) {
		var jadeFiles = [];
		_.forEach(files, function(n, key) {
			if ( path.extname(files[key].path) === '.jade' ) {
				jadeFiles.push(files[key].path);
			}
		});
		readManyJadeFiles(jadeFiles, dest, compileJade);
	};
	return false;
};
...
function compileJade (jadeStr, fileName, dest , many) {
	var isPretty;
	...
	var fn = jade.compile(jadeStr, {pretty: isPretty});
	var html = fn()
...
function readJadeFile (src, dest, callback) {
	var fileName = path.basename(src, '.jade');
...
function readManyJadeFiles (array, dest, callback) {
